...
preProcValues <- preProcess(df_interp[,-4], method = c("scale"))
df_knn <- predict(preProcValues, df_interp)

# Split training/test data sets
inTraining <- createDataPartition(df_knn$life_expectancy, p = 0.75, list = FALSE)
knn_train  <- df_knn[inTraining,]
knn_test   <- df_knn[-inTraining,]
...
# Split training/test data sets
inTraining <- createDataPartition(df_interp$life_expectancy, p = 0.75, list = FALSE)
tree_train  <- df_interp[inTraining,]
tree_test   <- df_interp[-inTraining,]
```
