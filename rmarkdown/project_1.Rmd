...
We next moved into looking at correlation (Figure \@ref(fig:corr-1)). We began examining various correlation matrices to determine whether there were variable(s) that should be addressed, and started by removing redundant variables and picking ones with fewer `NA` values (Figure \@ref(fig:corr-2). 

Finally, we looked at what happens when we also remove `population`, since it has minimal correlation to `life_expectancy` (Figure \@ref(fig:corr-3).

```{r correlation, message=FALSE, warning=FALSE, echo=FALSE, fig.show='hide'}
...
Our next task was to address the missing values in the data set (Figure \@ref(fig:missing-1)). We dropped all rows where life expectancy is `NA`, and limited the scope of our analysis to not include these countries (Figure \@ref(fig:missing-2)).
...
In looking at the relationship between Hepatitis B and Life Expectancy (Figure \@ref(fig:hep-life)), our options are to either drop all of the `NA`'s, impute the values, or fill with `0`'s. For our interpretable model, we made the decision to drop the `hepatitis_b` variable along with the remainder of the `NA`'s and we will revisit them for our predictive model (Figure \@ref(fig:missing-3)).
...
```{r corr-1, fig.cap="Variable Correlation, Original", echo=FALSE}
knitr::include_graphics(here::here("images", "correlation 1.png"))
```

```{r corr-2, fig.cap="Variable Correlation, 1st Iteration", echo=FALSE}
knitr::include_graphics(here::here("images", "correlation 2.png"))
```

```{r corr-3, fig.cap="Variable Correlation, 2nd Iteration", echo=FALSE}
knitr::include_graphics(here::here("images", "correlation 3.png"))
```

```{r missing-1, fig.cap="Missing Values Analysis, pt. 1", echo=FALSE}
knitr::include_graphics(here::here("images", "missing-1.png"))
```

```{r missing-2, fig.cap="Missing Values Analysis, pt. 2", echo=FALSE}
knitr::include_graphics(here::here("images", "missing-2.png"))
```

```{r hep-life, fig.cap="Relationship between Hepatitis B and Life Expectancy", echo=FALSE}
knitr::include_graphics(here::here("images", "hepatitis-lifeexp.png"))
```

```{r missing-3, fig.cap="Missing Values Analysis, pt. 3", echo=FALSE}
knitr::include_graphics(here::here("images", "missing-3.png"))
```
