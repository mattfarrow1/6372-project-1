...
## Ridge Regression - Imputed

Run ridge regression and LASSO again using imputed data set.
...
The top 4 variables using LASSO on the imputed data set are similar to the non-imputed data set: `income_composition_of_resources`,`statusDeveloped`, `schooling`, and `hiv_aids`. At this point, proceed with model using imputed data set.

## LASSO - Include Country
...
```{r}
#build a linear model using the top 4 variables from LASSO along with country for both imputed data set and removed NA data set
...
par(mfrow = c(1, 1))
plot(
  predict_model1$fitted.values,
  df_predict2$life_expectancy,
...
#check rows 864, 1875, 1126 as they show higher residuals/leverage
df_predict2[c(864,1875,1126),c(1:4,14,17)]

```

# Remove Haiti Outlier - 2010 was the year of the devastating earthquake and Eritrea - war was ongoing in 2000

```{r}
df_predict3 <- df_predict2[-c(1126,864),]
